...
export default function HermiosPage() {
  return (
    <div className="w-full overflow-x-hidden bg-[#FFFF00] font-sans text-black antialiased">
      {/* SECTION 1 — HERO */}
      <section className="mx-auto flex max-w-[1280px] flex-wrap items-center gap-16 px-6 pb-24 pt-24 md:px-12 md:pt-[120px] md:pb-[100px]">
        <Reveal direction="left" className="min-w-[320px] flex-1 basis-[460px]">
          <h1 className="m-0 mb-4 text-[38px] font-bold leading-[1.12] tracking-[-0.02em] text-black md:text-[52px]">
            DealiOS
            <br />
            <span className="text-[22px] font-semibold text-[#3D3D1A] md:text-[28px]">
          ...
          <p className="m-0 mb-9 max-w-[480px] text-lg leading-[1.6] text-[#3D3D1A]">
          ...
          <p className="m-0 mt-4.5 text-sm leading-[1.5] text-[#4A4A2A]">
            Sin compromiso.
            <br />
        ...
        <Reveal direction="right" className="relative h-[460px] min-w-[340px] flex-1 basis-[520px]">
          ...
          <div className="absolute bottom-[-18px] right-[4%] h-[220px] w-[108px] rounded-[22px] border border-white/10 bg-[#FFFF00] p-2 shadow-[0_20px_40px_-14px_rgba(0,0,0,0.5)]">
      ...
      <InmobiliariasSection bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <AutomotorasSection bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <HospitalitySection bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <ArquitecturaSection bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <GaleriaSection bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <PrimeraImpresion bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <Reveal direction="left">
        <OpportunitiesSection bg="#FFFF00" text="#000000" />
      </Reveal>

      <ProblemSection bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <CasosReales bg="#FFFF00" text="#000000" />

      <NuestroEnfoque bg="#FFFF00" text="#000000" />

      <PuntaDelEste bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      <TuNegocio bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      {/* SECTION 3 — FEATURES */}
      <section className="bg-[#FFFF00] px-6 py-24 md:px-12">
        <Reveal direction="right" className="mx-auto max-w-[1200px]">
          <h2 className="m-0 mb-12 max-w-[640px] text-[28px] font-bold tracking-[-0.01em] text-black md:text-[34px]">
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6">
            {featureCards.map((card) => (
              ...
              >
                ...
                <h3 className="m-0 mb-2.5 text-xl font-semibold text-black md:text-[21px]">{card.title}</h3>
                <p className="m-0 text-[15px] leading-[1.6] text-[#3D3D1A]">{card.desc}</p>
              </div>
            ))}
          </div>
        </Reveal>
      ...
      <section className="bg-[#FFFF00] px-6 py-24 md:px-12">
        <Reveal direction="left" className="mx-auto max-w-[1200px]">
          <h2 className="m-0 mb-12 text-[28px] font-bold tracking-[-0.01em] text-black md:text-[34px]">
          ...
          <div className="flex flex-col gap-4.5">
            {industryCards.map((card) => (
              ...
              >
                <div className="min-w-[200px] text-xl font-semibold text-black">{card.title}</div>
                <div className="hidden self-stretch bg-white/10 md:block md:w-px" />
                <p className="m-0 min-w-[260px] flex-1 text-base leading-[1.5] text-[#3D3D1A]">
                  {card.desc}
                </p>
              </div>
            ))}
      ...
      <section className="bg-[#FFFF00] px-6 py-24 md:px-12">
        <Reveal direction="right" className="mx-auto max-w-[1200px]">
          <h2 className="m-0 mb-10 text-[28px] font-bold tracking-[-0.01em] text-black md:text-[34px]">
            Mirá Cómo Funcionan Nuestros Sistemas
          </h2>
          <ProductTabs text="#000000" />
        </Reveal>
      </section>
      ...
      <section className="bg-[#FFFF00] px-6 py-24 md:px-12">
        <Reveal direction="left" className="mx-auto max-w-[1100px] text-center">
          <h2 className="m-0 mb-11 text-[28px] font-bold tracking-[-0.01em] text-black md:text-[34px]">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {pricingTiers.map((tier) => (
              ...
              >
                <h3 className="m-0 mb-6 text-xl font-semibold tracking-[-0.01em] text-black">{tier.name}</h3>
                <div className="mb-7 flex flex-col gap-3">
                  {tier.features.map((feature) => (
                    <div key={feature} className="flex items-center gap-3 text-[16px] text-black">
                ...
                <div className="mt-auto border-t border-white/10 pt-7">
                  {tier.price !== "A definir" && (
                    <div className="mb-1.5 text-sm text-[#3D3D1A]">Desde</div>
                  )}
                  <div className="mb-1 text-[32px] font-bold tracking-[-0.01em] text-black">
                    {tier.price}
                    {tier.priceSuffix && (
                      <span className="text-lg font-medium text-[#3D3D1A]"> {tier.priceSuffix}</span>
                    )}
                  </div>
                  {tier.period && (
                    <div className="mb-7 text-base text-[#3D3D1A]">{tier.period}</div>
                  )}
      ...
      <FAQSection bg="#FFFF00" text="#000000" muted="#3D3D1A" />

      {/* SECTION 7 — CTA */}
      <section id="assessment" className="bg-[#FFFF00] px-6 py-[120px] text-center md:px-12">
        <Reveal direction="right" className="mx-auto max-w-[640px]">
          <h2 className="m-0 mb-5.5 text-[32px] font-bold tracking-[-0.01em] text-black md:text-[38px]">
            Analicemos tu presencia digital.
          </h2>
          <p className="m-0 mb-10 text-lg leading-[1.6] text-[#3D3D1A]">
      ...
      {/* FOOTER */}
      <footer className="border-t border-white/10 bg-[#FFFF00] px-6 py-14 md:px-12">
        <div className="mx-auto flex max-w-[1200px] flex-wrap items-center justify-between gap-6">
          <div className="text-lg font-bold tracking-[-0.01em] text-black">Pegasus Pixels</div>
          <nav className="flex flex-wrap gap-7">
            {footerNav.map((item) => (
              <a
                key={item}
                href="#"
                className="text-[15px] text-[#3D3D1A] no-underline transition-colors hover:text-[#2563EB]"
              >
                {item}
              </a>
            ))}
          </nav>
          <div className="flex flex-wrap gap-5 text-sm text-[#3D3D1A]">
            <a
              href="mailto:hello@pegasuspixels.com"
              className="text-[#3D3D1A] no-underline transition-colors hover:text-[#2563EB]"
            >
              hello@pegasuspixels.com
            </a>
            <a href="#" className="text-[#3D3D1A] no-underline transition-colors hover:text-[#2563EB]">
              WhatsApp
            </a>
          </div>
        </div>
        <div className="mx-auto mt-8 max-w-[1200px] text-[13px] text-[#4A4A2A]">
